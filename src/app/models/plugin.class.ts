...
class Country {
  ...
  /* constructor(name, iso2, dialCode, priority = 0, areaCodes = null) {
    this.name = name;
    this.iso2 = iso2;
...
export class Plugin {
  public telInput: ElementRef;
  public options: Options;
  // ns: string; (depend on var pluginName + var id)
  public isGoodBrowser: boolean;
  public hadInitialPlaceholder: boolean;

  private countries: Country[] = [];
  private countryCodes: any;
  private preferredCountries: any;
  private defaultCountry: string;
  private selectedCountryData;

  constructor(@Inject(DOCUMENT) private document: any, element: ElementRef, options: Options) {
    this.telInput = element;
    this.options = options;
    this.isGoodBrowser = element.nativeElement.setSelectionRange as boolean;
    this.hadInitialPlaceholder = element.nativeElement.hasAttribute('placeholder');
  }
  ...
  public _init() {
    ...
    const isMobile = /Android.+Mobile|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(window.navigator.userAgent);

    if (isMobile) {
      // trigger the mobile dropdown css
      this.document.body.classList.add('iti-mobile');
    ...
    // we return these deferred objects from the _init() call so they can be
    // watched, and then we resolve them when each specific request returns
   ...
   * IMPLEMENT THE DOM MANIPULATION
   * FUNCTIONS BELOW DIRECTLY IN
   * THE HTML TEMPLATE */

   /*
  // generate all of the markup for the plugin: the selected flag overlay, and the dropdown
  private _generateMarkup() {
    // prevent autocomplete as there's no safe, cross-browser event
    // we can react to, so it can easily put the plugin in an inconsistent
    // state e.g. the wrong flag selected for the autocompleted number,
    // which on submit could mean the wrong number is saved (esp in nationalMode)
    this.telInput.nativeElement.setAttribute("autocomplete", "off");
  ...
  private _appendListItems(countries, className) {
  ...
  */
  ...
  private _setInitialState() {
    const val = this.telInput.nativeElement.value;
  ...
  private _initListeners () {
    ...
    // PS: HIDDENINPUT MOST LIKELY WILL COME
  ...
  /** TODO: UPDATE THIS FUNCTION ONCE ALL
   * SETUP WITH ANGULAR FORMS IS MADE
   * (OR ADD THIS CODE TO THE FORM
   * SUBMIT FUNCTION)
   */
  ...
  /** TODO: UPDATE THIS FUNCTION USING
   * EVENT LISTENERS IN THE HTML TEMPLATE
   */
  // initialise the dropdown listeners
  private _initDropdownListeners() {
    const that = this;

    // hack for input nested inside label:
    // clicking the selected-flag to open the dropdown
    // would then automatically trigger a 2nd click on
    // the input which would close it again
    const label = this.telInput.closest("label");
    if (label.length) {
      label.on("click" + this.ns, function(e) {
        // if the dropdown is closed, then focus the input, else ignore the click
        // PS: COUNTRYLIST MOST LIKELY WILL COME FROM THE HTML TEMPLATE
    ...
    // PS: SELECTEDFLAGINNER MOST LIKELY WILL COME FROM THE HTML TEMPLATE
    ...
    if ($.fn[pluginName].autoCountry) {
      ...
      if (typeof this.options.geoIpLookup === 'function') {
        this.options.geoIpLookup(function(countryCode) {
          ...
          // UPDATE: use setTimeout in case their geoIpLookup
          // function calls this callback straight away (e.g. if
          // they have already done the geo ip lookup somewhere else).
          // Using setTimeout means that the current thread of execution
          // will finish before executing this, which allows the plugin to finish initialising.
          setTimeout(function() {
  ...
  private _cap(number: string) {
    var max = this.telInput.nativeElement.getAttribute('maxlength');
    return (max && number.length > max) ? number.substr(0, max) : number;
  }

  // listen for mousedown, focus and blur
  private _initFocusListeners() {
    var that = this;

    // mousedown decides where the cursor goes, so if
    // we're focusing we must preventDefault as we'll
    // be inserting the dial code, and we want the cursor
    // to be at the end no matter where they click
    this.telInput.on("mousedown" + this.ns, function(e) {
      if (!that.telInput.is(":focus") && !that.telInput.nativeElement.value) {
        e.preventDefault();
    ...
    // made the decision not to trigger blur() now,
    // because would only do anything in the case where
    // they manually set the initial value to just a dial
    // code, in which case they probably want it to be displayed.
  }

  private _removeEmptyDialCode() {
    var value = this.telInput.nativeElement.value,
  ...
  private _handleUpDownKey(key: number) {
  ...
  private _searchForCountry(query: string) {
    for (var i = 0; i < this.countries.length; i++) {
  ...
  private _updateValFromNumber(number: string) {
    ...
    this.telInput.nativeElement.value = number;
  }
  ...
  // (expects the number to contain an international dial code)
  ...
  private _highlightListItem(listItem) {
  ...
  private _getCountryData(countryCode: string, ignoreOnlyCountriesOption: boolean, allowFail: boolean) {
    var countryList = (ignoreOnlyCountriesOption) ? this.allCountries : this.countries;
  ...
  private _selectListItem(listItem) {
    ...
    if (this.isGoodBrowser) {
      var len = this.telInput.nativeElement.value.length;
  ...
  private _scrollTo(element, middle) {
  ...
  private _updateDialCode(newDialCode, hasSelectedListItem) {
    var inputVal = this.telInput.nativeElement.value,
      newNumber;
    ...
    this.telInput.nativeElement.value = newNumber;
  }
  ...
  private _getFullNumber() {
    var val = $.trim(this.telInput.nativeElement.value),
  ...
  private _beforeSetNumber(number: string) {
    if (this.options.separateDialCode) {
      ...
      if (dialCode) {
        ...
        var start = (number[dialCode.length] === " " || number[dialCode.length] === "-") ?
          dialCode.length + 1 :
          dialCode.length;
  ...
  handleUtils() {
    ...
    if (window.intlTelInputUtils) {
      ...
      if (this.telInput.nativeElement.value) {
